...
SERVICE_NAME="schoolbell.service"
NGINX_SITE="/etc/nginx/sites-available/schoolbell"
...
need_root() {
  if [[ "${EUID}" -ne 0 ]]; then
    die "Uruchom ten skrypt przez sudo lub jako root."
  fi
}

detect_mode() {
  if [[ -d "$APP_DIR" ]] || [[ -f "/etc/systemd/system/$SERVICE_NAME" ]] || [[ -f "$NGINX_SITE" ]]; then
    echo
    echo "Wykryto istniejącą instalację Schoolbell."
    echo "1) Aktualizacja bez kasowania config.json"
    echo "2) Nadpisz aplikację i konfigurację nginx/systemd"
    echo "3) Anuluj"
    read -r -p "Wybierz opcję [1/2/3]: " choice
...
backup_existing() {
  if [[ "$MODE" == "update" || "$MODE" == "force" ]]; then
    log "Tworzę backup istniejących plików"
    mkdir -p "$BACKUP_DIR"

    [[ -f "$APP_DIR/app.py" ]] && cp "$APP_DIR/app.py" "$BACKUP_DIR/app_$TS.py" || true
    [[ -f "$APP_DIR/config.json" ]] && cp "$APP_DIR/config.json" "$BACKUP_DIR/config_$TS.json" || true
    [[ -f "$NGINX_SITE" ]] && cp "$NGINX_SITE" "$BACKUP_DIR/nginx_schoolbell_$TS.conf" || true
    [[ -f "/etc/systemd/system/$SERVICE_NAME" ]] && cp "/etc/systemd/system/$SERVICE_NAME" "$BACKUP_DIR/$SERVICE_NAME.$TS" || true
  fi
}

install_packages() {
  log "Instaluję wymagane pakiety systemowe"
  export DEBIAN_FRONTEND=noninteractive
  apt-get update
  apt-get install -y python3 python3-pip python3-rpi.gpio nginx curl ca-certificates
}

prepare_dirs() {
  log "Tworzę katalogi aplikacji"
  mkdir -p "$APP_DIR" "$APP_DIR/uploads" "$APP_DIR/config_backups" "$APP_DIR/logs" "$BACKUP_DIR"
  chown -R "$APP_USER:$APP_GROUP" "$APP_DIR"
  chmod 755 "$APP_DIR" "$APP_DIR/uploads" "$APP_DIR/config_backups" "$APP_DIR/logs" "$BACKUP_DIR"
}

install_python_deps() {
  log "Instaluję zależności Python"
  pip3 install --break-system-packages flask werkzeug || pip3 install flask werkzeug
}

write_app() {
  log "Tworzę plik app.py"
...
write_config() {
  ...
  if [[ -f "$config_path" && "$MODE" != "force" ]]; then
    log "config.json już istnieje - zostawiam bez zmian"
    return
  fi

  log "Tworzę domyślny config.json"
  cat > "$config_path" <<EOF
{
  ...
  "bell": {
    "manual_short": 5,
    "manual_long": 10,
    "duration_lesson_end": 5,
    "duration_break_end": 5,
    "ring_on_transitions": true
  },
  "button": {
    "enabled": false,
    "pin": 27,
  ...
  "timetables": {
    "normal": {
      "day_start": "08:30",
      "lessons": [45, 45, 45, 45, 45, 45],
      "breaks": [10, 10, 10, 10, 10]
    }
  }
}
EOF
...
server {
    ...
    location / {
        proxy_pass http://$LOCAL_HOST:$APP_PORT;
...
EOF
  ...
  ln -sf "$NGINX_SITE" /etc/nginx/sites-enabled/schoolbell
...
configure_wifi_powersave() {
  log "Wyłączam oszczędzanie energii Wi‑Fi w NetworkManager"
...
create_service() {
  log "Tworzę usługę systemd"
  cat > "/etc/systemd/system/$SERVICE_NAME" <<EOF
[Unit]
Description=School Bell Web Panel
...
summary() {
  ...
  echo "Tryb:           $MODE"
  echo "Aplikacja:      $APP_DIR/app.py"
  echo "Usługa:         $SERVICE_NAME"
  echo "Adres WWW:      http://${ip:-<IP_RASPBERRY>}/"
  echo "Upload logo:    /logo/upload"
  echo "Katalog logo:   $APP_DIR/uploads"
  echo "GPIO:           22 (HIGH)"
  echo "Backup:         $BACKUP_DIR"
  if [[ -n "$AUTH_TOKEN" ]]; then
    echo "Token:          ustawiony z argumentu"
  elif [[ -f "$APP_DIR/config.json" ]]; then
    echo "Token:          pozostawiony z istniejącego configu lub pusty"
  fi
  echo "=========================================="
  echo
  echo "Kontrola:"
  echo "  sudo systemctl status $SERVICE_NAME --no-pager"
  echo "  sudo journalctl -u $SERVICE_NAME -n 80 --no-pager -l"
